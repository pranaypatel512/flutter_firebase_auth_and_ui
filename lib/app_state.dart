...
class ApplicationState extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    .where('attending',isEqualTo: true).snapshots().listen((event) {
      _attendees=event.docs.length;
      notifyListeners();
     });
    ...
    FirebaseAuth.instance.userChanges().listen((user) {
      if (user != null) {
        ...
        _attendingSubscription = FirebaseFirestore.instance.collection('attendees').doc(user.uid).snapshots().listen((event) {
          if(event.data()!=null){
            if(event.data()!['attending'] as bool){
